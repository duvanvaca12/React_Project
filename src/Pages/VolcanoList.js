...
export default function VolcanoList(){
    ...
    let queryCountry = (populated === 0) ? `${volcanoByName}?country=${inputCountry.toLowerCase()}` :
    `${volcanoByName}?country=${inputCountry.toLowerCase()}&populatedWithin=${populated}km`
    
    // Asynchronously function

    useEffect(() =>{fetchData(urlCountries, setCountries)}, [])
    useEffect(() =>{fetchData(queryCountry,setVolcanoData)}, [queryCountry]); 
